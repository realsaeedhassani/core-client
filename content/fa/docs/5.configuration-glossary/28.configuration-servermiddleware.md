...
Because connect itself is a middleware, registered middleware will work with both `nuxt start` and also when used as a middleware with programmatic usages like [express-template](https://github.com/nuxt-community/express-template). Nuxt [Modules](/___documentation___directory-structure/modules) can also provide `serverMiddleware` using [this.addServerMiddleware()](/___documentation___internals-glossary/internals-module-container#addservermiddleware-middleware)
