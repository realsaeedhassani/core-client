...
If your `fetch()` accepts a `context` argument, it will be treated like a legacy fetch hook. This functionality is deprecated, and should be replaced with either `asyncData` or an [anonymous middleware.](/___documentation___directory-structure/middleware#anonymous-middleware)
::
...
::alert{type="info"}
You can access the Nuxt [context](/___documentation___concepts/context-helpers) within the fetch hook using `this.$nuxt.context`.
::
...
::alert{type="warning"}
`asyncData` is only available for [pages](/___documentation___directory-structure/pages) and you don't have access to `this` inside the hook.
::
...
::alert{type="next"}
Learn more about the [watchQuery property](/___documentation___components-glossary/watchquery) and see the list of available [keys in context](/___documentation___concepts/context-helpers).
::
