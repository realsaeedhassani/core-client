...
You may want to tweak the [optimization configuration](/___documentation___configuration-glossary/configuration-build#optimization) a bit, avoiding a rewrite of the default object.
...
With Nuxt, you can also control plugins execution context: if they are meant to be run on the `client` or in the `server` builds (or differentiating `dev` and `prod` builds) within [`build.extend`](/___documentation___configuration-glossary/configuration-build#extend), where you can manually pass webpack plugins too.
...
::alert{type="next"}
The `nuxt.config.js` has way more customization and configuration options! Check out all its keys in the [configuration glossary](/___documentation___configuration-glossary/configuration-build).
::
