...
![](/img/___documentation___context.svg)

The `context` object is available in specific Nuxt functions like [asyncData](/___documentation___features/data-fetching#async-data), [plugins](/___documentation___directory-structure/plugins), [middleware](/___documentation___directory-structure/middleware) and [nuxtServerInit](/___documentation___directory-structure/store#the-nuxtserverinit-action). It provides _additional_ and often optional information about the current request to the application.
...
Learn more about the different context keys in our [Internals Glossary](/___documentation___internals-glossary/context)

### Using page parameters for your API query

The context directly exposes possible dynamic parameters of the route via `context.params`. In the following example, we call an API via the `nuxt/http` module using a dynamic page parameter as part of the URL. Modules, like the [nuxt/http](https://http.nuxtjs.org/) module, can expose own functions which are then available through the [context.app](/___documentation___internals-glossary/context#app) object.
...
Want to use query parameters instead? You then use [context.query.id](/___documentation___internals-glossary/context#query).

### Redirecting users & accessing the store

Accessing the [Vuex store](/___documentation___directory-structure/store) (when you have it set up through the `store` directory) is also possible through the context. It provides a `store` object which can be treated as `this.$store` in Vue components. In addition, we use the `redirect` method, an exposed helper through the context, to redirect the user in case the `authenticated` state is [falsy](https://developer.mozilla.org/en-US/___documentation___Glossary/Falsy).
...
::alert{type="next"}
Check out the Internals Glossary book for more examples of the [redirect method](/___documentation___internals-glossary/context#redirect)
::
...
Read more in the corresponding [loading feature chapter](/___documentation___features/loading)
