...
::alert{type="warning"}
The `<Nuxt>` component can only be used inside [layouts](/___documentation___concepts/views#layouts).
::
...
`<NuxtLink>` also comes with [smart prefetching](/___documentation___features/nuxt-components#the-nuxtlink-component) out of the box.
