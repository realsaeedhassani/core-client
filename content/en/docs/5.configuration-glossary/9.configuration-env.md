...
::alert
For nuxt versions > 2.12+, in cases where environment variables are required at runtime (not build time) it is recommended to replace the `env` property with [runtimeConfig properties](/___documentation___configuration-glossary/configuration-runtime-config): `publicRuntimeConfig` and `privateRuntimeConfig`. 
<br>
...
2. Via `context.env.baseUrl`, see [context API](/___documentation___internals-glossary/context).
...
As [serverMiddleware](/___documentation___configuration-glossary/configuration-servermiddleware) is decoupled from the main Nuxt build, `env` variables defined in `nuxt.config.js` are not available there.
