...
If you want to have a redirect to `router.base`, you can do so [using a Hook, see _Redirect to router.base when not on root_](/___documentation___configuration-glossary/configuration-hooks#redirect-to-routerbase-when-not-on-root).
...
Globally configure [`<nuxt-link>`](/___documentation___features/nuxt-components#the-nuxtlink-component) default active class.
...
Globally configure [`<nuxt-link>`](/___documentation___features/nuxt-components#the-nuxtlink-component) default exact active class.
...
Globally configure [`<nuxt-link>`](/___documentation___features/nuxt-components#the-nuxtlink-component) default prefetch class (feature disabled by default)
...
To learn more about the middleware, see the [middleware guide](/___documentation___directory-structure/middleware#router-middleware).
...
Configure `<nuxt-link>` to prefetch the _code-splitted_ page when detected within the viewport. Requires [IntersectionObserver](https://developer.mozilla.org/en-US/___documentation___Web/API/Intersection_Observer_API) to be supported (see [Caniuse](https://caniuse.com/#feat=intersectionobserver)).
...
> Added with v2.13.0, only available for [static target](/___documentation___features/deployment-targets#static-hosting).
