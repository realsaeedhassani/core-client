...
- Type: `Object`
- See `babel-loader` [options](https://github.com/babel/babel-loader#options) and `babel` [options](https://babeljs.io/___documentation___en/options)
- Default:
...
::alert{type="warning"}
**Warning:** The `isClient` and `isServer` keys provided in are separate from the keys available in [`context`](/___documentation___internals-glossary/context). They are **not** deprecated. Do not use `process.client` and `process.server` here as they are `undefined` at this point.
::
...
> Nuxt allows you provide your own templates which will be rendered based on Nuxt configuration. This feature is specially useful for using with [modules](/___documentation___directory-structure/modules).
...
Templates are rendered using [`lodash.template`](https://lodash.com/___documentation___#template) you can learn more about using them [here](https://github.com/learn-co-students/javascript-lodash-templates-lab-v-000).
...
In this example, `ky` will be transpiled by Babel if Nuxt is not in [modern mode](/___documentation___configuration-glossary/configuration-modern).
...
> You can provide your custom files to watch and regenerate after changes. This feature is specially useful for using with [modules](/___documentation___directory-structure/modules).
